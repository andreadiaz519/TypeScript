...
Implementa la función para enviar correos a dos destinatarios distinto */
...
function enviarCorreo(destinatario: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      const exito = Math.random() > 0.5; // Simula éxito o error
...
// Enviar correos a dos destinatarios
enviarCorreo("andrea@example.com")
